...
#                                                                              #
#==============================================================================#
"""xSimpleBahroStone.py


Displays a very simple bahro stone, instead of making the script yourself.
...
destinationAge = ptAttribString    ( 2, "Age to link to", "")
...
class xSimpleBahroStone(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        if destinationAge.value == "fakelink":
            print "Bahro Stone: this book links to this same Age, fetching spawnpoint for fakelink."
            try:
    ...
    def OnNotify(self, state, id, events):
        if id == bookClickable.id:
            if PtWasLocallyNotified(self.key) and state:
                print "Bahro Stone: you just clicked one."
                self.IPickupBook()
        
        else:
            for event in events:
                if event[0] == PtEventType.kBook:
                    print "Bahro Stone: got notify from interface, event=%d, id=%d" % (event[1], event[2])
                    
                    if event[1] == PtBookEventTypes.kNotifyImageLink:
                        print "Bahro Stone: hit linking panel n#%s" % event[2]
    ...
    def AutoFindLinkingPanel(self, age, sp=None):
        ...
        if image != None:
            booksDustGlobal.bhrLinkPanelMap.textmap.drawImage(50, 60, image, 0)
            print "Bahro Stone: finding a linking panel was successfull."
        else:
    ...
    def ILink(self):
        if destinationAge.value == "fakelink":
            print "Bahro Stone: We're linking to the same Age. Using fakelink."
            ...
            print "Bahro Stone: time to link !"
    ...
    def IShowLinkingBook(self):
        ...
        if destinationAge.value.lower() == 'void':
            ...
            bookContent += '<pb><img src="xlinkpanelblackvoid*1#0.hsm" opacity=0.7 align=center blend=alpha>'
            ...
            if panelName:
                if panelName.endswith(".bik"):
                    ...
                    bookContent += '<pb><img src="%s" opacity=0.7 align=center link=1 blend=alpha>' % panelName
            else:
                print "Bahro Stone: No link panel texture given. Will attempt to find one when book is shown."
                bookContent += '<pb><img src="xlinkpanelblackvoid*1#0.hsm" opacity=0.7 align=center link=1 blend=alpha>'
        ...
        BookGUI.setSize(1, 1)
        BookGUI.setGUI("bkBahroRockBook")
        BookGUI.show(1)
        
        
    def OnTimer(self, id):
        print "Bahro Stone: got timer callback id %d" % id
        
        if id == 1:
            print "Bahro Stone: Fading in..."
            PtFadeIn(1.5, 1)
            bookClickable.enable()
