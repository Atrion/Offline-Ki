...
class xAgeSDLBoolCondResp(ptResponder,):
    ...
    def OnServerInitComplete(self):
        ...
        if (ageSDL[strSDLVar.value][0] == boolOnTrue.value):
            ...
            PtDebugPrint(('DEBUG: xAgeSDLBoolCondResp.OnServerInitComplete():\tRunning responder on %s, fastforward=%d' % (self.sceneobject.getName(),
             boolInitFF.value)))
        self.initFinished = 1
    ...
    def OnNotify(self, state, id, events):
        if (self.initFinished != 1):
            return 
        if self.invalidVarName:
            PtDebugPrint(('DEBUG: xAgeSDLBoolCondResp.OnNotify():\tRunning responder on %s, fastforward=%d' % (self.sceneobject.getName(),
             boolInitFF.value)))
            return 
        ageSDL = PtGetAgeSDL()
