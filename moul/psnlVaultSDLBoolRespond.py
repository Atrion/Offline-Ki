...
class psnlVaultSDLBoolRespond(ptResponder,):
        ...
        try:
            ...
            if ((type(stringVarName.value) == type('')) and (stringVarName.value != '')):
                if ageSDL[stringVarName.value][0]:
                    PtDebugPrint(('DEBUG: psnlVaultSDLBoolRespond.IFinishInit():\tRunning true responder on %s, fastforward=%d' % (self.sceneobject.getName(),
                     boolFFOnInit.value)))
                    respBoolTrue.run(self.key, fastforward=boolFFOnInit.value)
                else:
                    PtDebugPrint(('DEBUG: psnlVaultSDLBoolRespond.IFinishInit():\tRunning false responder on %s, fastforward=%d' % (self.sceneobject.getName(),
                     boolFFOnInit.value)))
    ...
    def runDefault(self):
        PtDebugPrint('ERROR: psnlVaultSDLBoolRespond: running internal default')
